...
export class BankPage {
    ...
    async enterDepositAmonut(amount :string){  
        const depositAmonutInput = this.page.locator(bankPageLocators.deposit.textboxs.amountInput);
        await depositAmonutInput.fill(amount);
    }

    async clickDepositConfirm() {
        const depositConfirmButton = this.page.locator(bankPageLocators.deposit.buttons.confirmButton);
    ...
    async getAccountDetails() {
        ...
        const accountId = await articleLocator.locator(bankPageLocators.title.lable.accountID).textContent();
        const name = await articleLocator.locator(bankPageLocators.title.lable.name).textContent();
        const balance = await articleLocator.locator(bankPageLocators.title.lable.balance).textContent();
    ...
    async verifyBalaceAfter(amount: string) {
        ...
        try {
            ...
            console.error(`Balance verification failed: expected ${expectedBalance}, but got ${actualBalance}.`);
