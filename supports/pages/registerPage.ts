...
export class RegisterPage {
    ...
    async clickRegisterLink(){
        await this.page.locator(registerPageLocators.lableLinks.registerLink).click();
        await this.page.waitForURL(`${process.env.CUBANK_WEB}/register`);
    }

    async fillRegisterForm(accountNumber:string, password:string, firstName:string, lastName:string) {
        // Input values into the textboxes
        const accountNumberInput = this.page.locator(registerPageLocators.textboxs.accountNumberInput);
        await accountNumberInput.fill(accountNumber);

        const passwordInput = this.page.locator(registerPageLocators.textboxs.passwordInput);
        await passwordInput.fill(password);
        
        const firstNameInput = this.page.locator(registerPageLocators.textboxs.firstNameInput);
